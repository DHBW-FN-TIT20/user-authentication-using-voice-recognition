...
\lstset{escapeinside={//(*@}{@*)}}
\begin{lstlisting}[language=HTML,caption=Auswahl der Audiodatei,label=lst:AudiodateiAuswahl]
...
\begin{lstlisting}[language=HTML,caption=Audio Tag,label=lst:AudioAppTsx]
...
\begin{lstlisting}[language=JavaScript,caption=login(),label=lst:LoginAppTsx]
...
\begin{lstlisting}[language=Python,caption=Parametervalidierung handle\_api\_request(),label=lst:ParameterValidationServerPy]
if not (
