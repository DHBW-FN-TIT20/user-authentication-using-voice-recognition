...
% \begin{lstlisting}[language=JavaScript,caption=Konfigurationsmöglichkeiten,label=configs]
% Configs = {
...
% Die Werte für die Anzahl und Länge der Frames \texttt{amount\_oftexttt_frames} und \texttt{sizetexttt_oftexttt_frame} berechnen sich aus vorherigen Versuchen, der verwendeten Abtastrate und der gewünschten Trainingsclip-Länge.


% Bei den Features gibt der \texttt{order} Parameter die Anzahl der Features pro Frame an.
...
% Der \texttt{weight} Parameter gibt lediglich an ob in dieser Konfiguration dieses Feature verwendet werden soll oder nicht.
